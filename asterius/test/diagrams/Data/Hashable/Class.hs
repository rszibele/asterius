...
import Foreign.Storable (alignment, peek, sizeOf, peekByteOff)
...
hashPtrWithSalt p len salt =
    ...
    fromIntegral `fmap` native_hashCString (castPtr p) (fromIntegral len)
    (fromIntegral salt)
...
native_hashCString :: CString -> CLong -> CLong -> IO CLong
native_hashCString str len salt = go len 0 salt
   where
      go 0 _   hash = return hash
      go l off hash = do
         c <- peekByteOff str off
         let hash' = (hash * 16777619) `xor` (fromIntegral (c :: Word8))
         go (l-1) (off+1) hash'
...
hashByteArrayWithSalt ba !off !len !h =
    ...
    fromIntegral $ native_hashByteArray ba (fromIntegral off) (fromIntegral len)
    (fromIntegral h)
...
-- | Native implementation
native_hashByteArray :: ByteArray# -> CLong -> CLong -> CLong -> CLong
native_hashByteArray ba off len salt = go len off# salt
   where
      I# off# =  fromIntegral off

      go 0 _   hash = hash
      go l off hash = go (l-1) (off +# 1#) hash'
         where
            c     = W8# (indexWord8Array# ba off)
            hash' = (hash * 16777619) `xor` (fromIntegral (c :: Word8))
